...
with codecs.open("金庸01飞狐外传.txt", 'r', encoding='utf-8')as f:
    for line in f.readlines():
      poss = pseg.cut(line)
      linesnames.append([])
      for w in poss:
...
cursor = db.cursor()
for name, id in zip(fei_name_node, range(1400,1600,1)):
    print(id,name,"fei")
      ...
      for v, w in edges.items():
        if w > 30:
          ...
          sql = "INSERT INTO TestModel_links(book, \
                     ...
                     VALUES ('%s','%s','%s',%s,%s,'%s')" % \
                ("shu", name, v,fei_name_nodes.index(name)+1400,fei_name_nodes.index(v)+1400,"待定")
