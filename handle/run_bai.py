...
with codecs.open("金庸06白马啸西风.txt", 'r', encoding='utf-8')as f:
    for line in f.readlines():
      poss = pseg.cut(line)
      linesnames.append([])
      for w in poss:
...
cursor = db.cursor()
for name, id in zip(fei_name_node, range(1000,1200,1)):
    print(id,name,"fei")
      ...
      for v, w in edges.items():
        if w > 20:
          ...
          sql = "INSERT INTO TestModel_links(book, \
                     ...
                     VALUES ('%s','%s','%s',%s,%s,'%s')" % \
                ("bai", name, v,fei_name_nodes.index(name)+1000,fei_name_nodes.index(v)+1000,"待定")
