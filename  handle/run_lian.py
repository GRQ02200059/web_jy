...
with codecs.open("金庸03连城诀.txt", 'r', encoding='utf-8')as f:
    for line in f.readlines():
      poss = pseg.cut(line)
      linesnames.append([])
      for w in poss:
...
cursor = db.cursor()
for name, id in zip(fei_name_node, range(600,800,1)):
    print(id,name,"fei")
      ...
      for v, w in edges.items():
        if w > 30:
          ...
          sql = "INSERT INTO TestModel_links(book, \
                     ...
                     VALUES ('%s','%s','%s',%s,%s,'%s')" % \
                ("lian", name, v,fei_name_nodes.index(name)+600,fei_name_nodes.index(v)+600,"待定")
