...
with codecs.open("金庸02雪山飞狐.txt", 'r', encoding='utf-8')as f:
    for line in f.readlines():
      poss = pseg.cut(line)
      linesnames.append([])
      for w in poss:
...
cursor = db.cursor()
for name, id in zip(fei_name_node, range(400,700,1)):
    print(id,name,"fei")
      ...
      for v, w in edges.items():
        if w > 5:
          ...
          sql = "INSERT INTO TestModel_links(book, \
                     ...
                     VALUES ('%s','%s','%s',%s,%s,'%s')" % \
                ("xue", name, v,fei_name_nodes.index(name)+400,fei_name_nodes.index(v)+400,"待定")
