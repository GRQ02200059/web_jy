...
with codecs.open("金庸10神雕侠侣.txt", 'r', encoding='utf-8')as f:
    for line in f.readlines():
      poss = pseg.cut(line)
      linesnames.append([])
      for w in poss:
...
cursor = db.cursor()
for name, id in zip(fei_name_node, range(1800,2000,1)):
    print(id,name,"fei")
      ...
      for v, w in edges.items():
        if w > 15:
          ...
          sql = "INSERT INTO TestModel_links(book, \
                     ...
                     VALUES ('%s','%s','%s',%s,%s,'%s')" % \
                ("shen", name, v,fei_name_nodes.index(name)+1800,fei_name_nodes.index(v)+1800,"待定")
