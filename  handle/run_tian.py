...
with codecs.open("金庸04天龙八部.txt", 'r', encoding='utf-8')as f:
    for line in f.readlines():
      poss = pseg.cut(line)
      linesnames.append([])
      for w in poss:
...
cursor = db.cursor()
for name, id in zip(fei_name_node, range(800,1000,1)):
    print(id,name,"fei")
      ...
      for v, w in edges.items():
        if w > 30:
          ...
          sql = "INSERT INTO TestModel_links(book, \
                     ...
                     VALUES ('%s','%s','%s',%s,%s,'%s')" % \
                ("tian", name, v,fei_name_nodes.index(name)+800,fei_name_nodes.index(v)+800,"待定")
