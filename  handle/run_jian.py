...
with codecs.open("金庸11侠客行.txt", 'r', encoding='utf-8')as f:
    for line in f.readlines():
      poss = pseg.cut(line)
      linesnames.append([])
      for w in poss:
...
cursor = db.cursor()
for name, id in zip(fei_name_node, range(2000,2200,1)):
    print(id,name,"fei")
      ...
      for v, w in edges.items():
        if w > 10:
          ...
          sql = "INSERT INTO TestModel_links(book, \
                     ...
                     VALUES ('%s','%s','%s',%s,%s,'%s')" % \
                ("jian", name, v,fei_name_nodes.index(name)+2000,fei_name_nodes.index(v)+2000,"待定")
